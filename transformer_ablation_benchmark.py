...
def main() -> int:
    ...
    # -------------------------------------------------------------------------
    # Build reference first, then copy identical parameters into all variants.
    # -------------------------------------------------------------------------
    ...
    for key in variant_keys:
        ...
        # This hint is safe only when the benchmark itself guarantees that
        # every token is valid. No runtime reduction is performed in forward.
        candidate.set_assume_all_valid_mask(
            args.padding_ratio == 0.0
        )

        models[key] = candidate

    # Move after copying to make copy logic simple and deterministic.
    for key in variant_keys:
        models[key] = (
            models[key]
            .to(
    ...
    print(
        ...
        f"mask-skip effective="
        f"{args.padding_ratio == 0.0}"
    )
    print(
        f"selected variants={','.join(variant_keys)}"
    )
    ...
    if failed_variants:
        ...
        if not args.benchmark_on_failure:
            print(
                ...
                "Use --benchmark-on-failure to time them anyway."
            )
            return 2
